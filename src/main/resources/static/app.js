...
function formatMetric(key, value) { if (key === 'successRate' || key === 'rollbackFrequency') return `${Math.round(value * 100)}%`; if (key === 'meanEndToEndLatencyMs' || key === 'meanTimeToRecoveryMs') return value >= 1000 ? `${(value / 1000).toFixed(1)}s` : `${Math.round(value)}ms`; return Math.round(value); }
