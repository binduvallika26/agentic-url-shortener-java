...
const headers = {'Content-Type': 'application/json', 'X-Actor': 'interview-candidate'};
...
function renderRun() {
  ...
  $('#advance').disabled = ['SUCCEEDED', 'ROLLED_BACK', 'SAFE_STOPPED'].includes(run.status); $('#approve').disabled = !waiting; $('#approve').dataset.step = waiting?.id || ''; $('#replan').disabled = false; $('#rollback').disabled = !Object.values(run.steps).some(step => step.status === 'SUCCEEDED'); renderEvidence();
}
function renderEvidence() {
  ...
  element.className = 'feed'; element.innerHTML = items.map(step => `<div class="event"><b>${step.id.toUpperCase()} · ${step.result.mode}${step.fallbackUsed ? ' · FALLBACK' : ''}</b><p>${step.result.summary}</p><p>Evidence: ${step.result.evidence.join(' · ')}</p><p>Risks: ${step.result.risks.join(' · ')}</p><p>Artifacts: ${step.result.artifacts.join(' · ')}</p></div>`).join('');
}
...
async function refresh() {
  const [metrics, audit] = await Promise.all([api('/api/metrics'), api('/api/audit')]); const labels = {totalRuns: 'Total runs', successRate: 'Successful completion', retries: 'Retry attempts', fallbacks: 'Fallback steps', rollbackFrequency: 'Runs rolled back', meanEndToEndLatencyMs: 'Mean end-to-end latency'}; const order = ['totalRuns', 'successRate', 'retries', 'fallbacks', 'rollbackFrequency', 'meanEndToEndLatencyMs'];
  $('#metrics').innerHTML = order.map(key => `<div class="metric"><b>${formatMetric(key, metrics[key])}</b><small>${labels[key]}</small></div>`).join(''); const element = $('#audit'); if (audit.length) { element.className = 'feed'; element.innerHTML = audit.slice(0, 12).map(event => `<div class="event"><b>${event.action} · ${event.outcome}</b><p>${event.actor} · ${new Date(event.occurredAt).toLocaleTimeString()} · ${event.correlationId.slice(0, 8)}</p></div>`).join(''); }
}
...
$('#launch').onclick = async () => { if (!validRequirement()) return; try { run = await api('/api/workflows', {method: 'POST', headers, body: JSON.stringify({scenario, requirement: $('#requirement').value.trim()})}); renderRun(); refresh(); toast('Workflow created with governed boundaries'); } catch (error) { toast(error.message); } };
...
$('#advance').onclick = async () => { try { run = await api(`/api/workflows/${run.id}/advance`, {method: 'POST', headers}); renderRun(); refresh(); toast(run.status === 'AWAITING_APPROVAL' ? 'Human decision required' : 'Ready agents executed'); } catch (error) { toast(error.message); } };
$('#approve').onclick = async () => { try { const step = $('#approve').dataset.step; run = await api(`/api/workflows/${run.id}/steps/${step}/approve`, {method: 'POST', headers, body: JSON.stringify({reason: 'Interview reviewer approved validated evidence'})}); renderRun(); toast(`${step} gate approved by human`); } catch (error) { toast(error.message); } };
$('#replan').onclick = async () => { if (!validRequirement()) return; try { run = await api(`/api/workflows/${run.id}/replan`, {method: 'POST', headers, body: JSON.stringify({requirement: $('#requirement').value.trim()})}); renderRun(); refresh(); toast(`Workflow replanned as revision ${run.revision}; prior outputs were invalidated`); } catch (error) { toast(error.message); } };
$('#rollback').onclick = async () => { try { run = await api(`/api/workflows/${run.id}/rollback`, {method: 'POST', headers, body: JSON.stringify({reason: 'Reviewer requested safe rollback'})}); renderRun(); refresh(); toast('Rollback completed and audited'); } catch (error) { toast(error.message); } };
$('#shorten').onclick = async () => { try { const link = await api('/api/links', {method: 'POST', headers, body: JSON.stringify({url: $('#longUrl').value, customCode: $('#alias').value})}); $('#linkResult').innerHTML = `Created <a style="color:#d7ff69" href="/${link.code}" target="_blank">${location.origin}/${link.code}</a> · visits ${link.visits}`; refresh(); toast('Short link persisted'); } catch (error) { toast(error.message); } };
