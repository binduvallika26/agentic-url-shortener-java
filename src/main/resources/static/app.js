...
function renderEvidence() {
  ...
  const final = run.finalSummary ? `<div class="event final-summary"><b>FINAL ENGINEERING SUMMARY</b><p>${escapeHtml(run.finalSummary)}</p></div>` : '';
  element.className = 'feed'; element.innerHTML = final + items.map(step => `<div class="event"><b>${escapeHtml(step.id.toUpperCase())} · ${escapeHtml(step.result.mode)}${step.fallbackUsed ? ' · FALLBACK' : ''}</b><p>${escapeHtml(step.result.summary)}</p><p>Evidence: ${step.result.evidence.map(escapeHtml).join(' · ')}</p><p>Risks: ${step.result.risks.map(escapeHtml).join(' · ')}</p>${step.result.artifacts.length ? `<p>Artifacts: ${renderArtifacts(step.result.artifacts)}</p>` : ''}</div>`).join('');
}
function formatMetric(key, value) { const number = Number(value); if (!Number.isFinite(number)) return key === 'successRate' || key === 'rollbackFrequency' ? '0%' : key.endsWith('Ms') ? '0ms' : '0'; if (key === 'successRate' || key === 'rollbackFrequency') return `${Math.round(number * 100)}%`; if (key === 'meanEndToEndLatencyMs' || key === 'meanTimeToRecoveryMs') return number >= 1000 ? `${(number / 1000).toFixed(1)}s` : `${Math.round(number)}ms`; return Math.round(number); }
async function refresh() {
  const [metrics, audit] = await Promise.all([api('/api/metrics'), api('/api/audit')]); const labels = {totalRuns: 'Total runs', successRate: 'Successful completion', replans: 'Replans', retries: 'Retry attempts', fallbacks: 'Fallback steps', rollbackFrequency: 'Runs rolled back', meanTimeToRecoveryMs: 'Mean time to recovery', meanEndToEndLatencyMs: 'Mean end-to-end latency'}; const order = ['totalRuns', 'successRate', 'replans', 'retries', 'fallbacks', 'rollbackFrequency', 'meanTimeToRecoveryMs', 'meanEndToEndLatencyMs'];
