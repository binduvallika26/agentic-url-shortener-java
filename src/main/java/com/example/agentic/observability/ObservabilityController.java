...
@RestController @RequestMapping("/api") public class ObservabilityController {
    ...
    @GetMapping("/metrics") public Map<String,Object> metrics(){var runs=workflows.all();var total=runs.size();var succeeded=runs.stream().filter(r->r.getStatus()==WorkflowModels.RunStatus.SUCCEEDED).count();var replans=runs.stream().flatMap(r->r.getDecisions().stream()).filter(d->d.kind().equals("run.replanned")).count();var retries=runs.stream().flatMap(r->r.getSteps().values().stream()).mapToInt(s->Math.max(0,s.getAttempts()-1)).sum();var fallbacks=runs.stream().flatMap(r->r.getSteps().values().stream()).filter(WorkflowModels.StepState::isFallbackUsed).count();var rollbacks=runs.stream().filter(r->r.getStatus()==WorkflowModels.RunStatus.ROLLED_BACK).count();var latency=runs.stream().filter(r->r.getCompletedAt()!=null).mapToLong(r->Duration.between(r.getStartedAt(),r.getCompletedAt()).toMillis()).average().orElse(0);var mttr=runs.stream().filter(r->r.getLastFailureAt()!=null&&r.getRecoveredAt()!=null).mapToLong(r->Duration.between(r.getLastFailureAt(),r.getRecoveredAt()).toMillis()).average().orElse(0);return Map.of("totalRuns",total,"successRate",total==0?0:(double)succeeded/total,"replans",replans,"retries",retries,"fallbacks",fallbacks,"rollbackFrequency",total==0?0:(double)rollbacks/total,"meanTimeToRecoveryMs",mttr,"meanEndToEndLatencyMs",latency);}
}
